...
static func _make_chair(prop: Dictionary) -> Node3D:
	...
	# Loop 240: soft seat pillow (not flat green fridge pad mid-FOV)
	_add_mesh_cyl(root, Vector3(0, 0.5, 0.04), 0.18, 0.1, mat_fab, false)
	_add_mesh_box(root, Vector3(0, 0.48, 0.04), Vector3(0.38, 0.08, 0.36), mat_fab)
	_add_mesh_cyl_rot(root, Vector3(0, 0.5, 0.22), 0.05, 0.36, mat_fab_l, Vector3(0, 0, PI * 0.5))
	for sx_s in [-1.0, 1.0]:
		_add_mesh_cyl_rot(root, Vector3(sx_s * 0.16, 0.5, 0.02), 0.04, 0.28, mat_fab_d, Vector3(PI * 0.5, 0, 0))
	...
	match seed0 % 3:
		...
		1:
			# Loop 240: cylinder-only balloon cushion (no flat green slab mid-FOV)
			_add_mesh_cyl(root, Vector3(0, 0.98, -0.1), 0.1, 0.42, mat_fab, false)
			_add_mesh_cyl_rot(root, Vector3(0, 1.18, -0.1), 0.055, 0.2, mat_fab, Vector3(0, 0, PI * 0.5))
			_add_mesh_cyl_rot(root, Vector3(0, 0.82, -0.1), 0.045, 0.18, mat_fab_d, Vector3(0, 0, PI * 0.5))
			for sx in [-1.0, 1.0]:
				_add_mesh_cyl(root, Vector3(sx * 0.08, 0.98, -0.1), 0.045, 0.36, mat_fab_d, false)
			for bi in 3:
				_add_mesh_cyl(root, Vector3(0, 0.84 + float(bi) * 0.12, -0.04), 0.01, 0.01, mat_fab_dd, false)
		_:
...
static func _make_armchair(prop: Dictionary) -> Node3D:
	## Victorian club / wing chair — solid-mat mahogany + velvet.
	## Loop 240: residual front ziggurat + rear fridge (morning_room_spawn/corner).
	## ONE continuous upholstered shell — no multi-box terraces, no stacked rear
	## horizontal cylinders (tank treads), no full-height outer organ pipes.
	## Silhouette = soft club body + short wing cheeks + single crown roll + air under seat.
	if prop.get("billboard", false) and prop.get("texture", "") != "":
		return _make_billboard_prop(prop)
	var root := Node3D.new()
	root.name = "Armchair"
	...
	var mat_fab_dd := _solid_matte(fabric.darkened(0.18), 0.94)
	var mat_fab_h := _solid_matte(fabric.darkened(0.03), 0.9)
	var mat_m := _solid_matte(Color(0.3, 0.14, 0.08), 0.72)
	var mat_md := _solid_matte(Color(0.18, 0.09, 0.05), 0.78)
	# ── Slim mahogany apron only (air under seat — not solid fridge plinth) ──
	_add_mesh_box(root, Vector3(0, 0.2, 0.04), Vector3(0.68, 0.035, 0.56), mat_md)
	_add_mesh_box(root, Vector3(0, 0.23, 0.04), Vector3(0.72, 0.018, 0.58), mat_m)
	# ── Seat: ONE soft pillow mass (cylinder-dominant, not stepped boxes) ──
	_add_mesh_cyl(root, Vector3(0, 0.36, 0.1), 0.26, 0.16, mat_fab, false)
	_add_mesh_box(root, Vector3(0, 0.34, 0.08), Vector3(0.54, 0.12, 0.42), mat_fab)
	_add_mesh_cyl_rot(root, Vector3(0, 0.36, 0.28), 0.07, 0.5, mat_fab_h, Vector3(0, 0, PI * 0.5))
	for sx_s in [-1.0, 1.0]:
		_add_mesh_cyl_rot(root, Vector3(sx_s * 0.24, 0.36, 0.06), 0.055, 0.34, mat_fab_h, Vector3(PI * 0.5, 0, 0))
	# Dark tufts only
	for bx in [-0.1, 0.0, 0.1]:
		for bz in [0.02, 0.14]:
			_add_mesh_cyl(root, Vector3(bx, 0.45, bz), 0.011, 0.008, mat_fab_dd, false)
	# ── Back: ONE continuous body (no terrace / no multi rear rolls) ──
	# Main shell — single tall mass, slightly set back
	_add_mesh_box(root, Vector3(0, 0.9, -0.02), Vector3(0.48, 0.82, 0.28), mat_fab)
	# Soft front cushion face (same family — not pale fridge door)
	_add_mesh_box(root, Vector3(0, 0.92, 0.1), Vector3(0.36, 0.64, 0.06), mat_fab_d)
	# Soft outer back curve — ONE vertical half-round only (not horizontal stack)
	_add_mesh_cyl(root, Vector3(0, 0.92, -0.14), 0.14, 0.78, mat_fab, false)
	# Crown: single thick wrap roll
	_add_mesh_cyl_rot(root, Vector3(0, 1.28, 0.0), 0.085, 0.5, mat_fab, Vector3(0, 0, PI * 0.5))
	_add_mesh_cyl_rot(root, Vector3(0, 1.32, 0.02), 0.05, 0.34, mat_fab_h, Vector3(0, 0, PI * 0.5))
	_add_mesh_box(root, Vector3(0, 1.34, -0.1), Vector3(0.1, 0.016, 0.03), mat_m)
	# ── Wings: short cheeks that merge into back (not tall fridge ears) ──
	for sx in [-1.0, 1.0]:
		# Wing body — shorter than back, continuous fabric
		_add_mesh_box(root, Vector3(sx * 0.28, 0.78, 0.06), Vector3(0.12, 0.58, 0.34), mat_fab)
		# Soft outer cheek (short vertical — stops above arm, not organ pipe)
		_add_mesh_cyl(root, Vector3(sx * 0.36, 0.78, 0.08), 0.06, 0.48, mat_fab, false)
		# Arm: horizontal bolster only (club-chair arm)
		_add_mesh_cyl_rot(root, Vector3(sx * 0.28, 0.48, 0.08), 0.06, 0.34, mat_fab_h, Vector3(PI * 0.5, 0, 0))
		_add_mesh_cyl(root, Vector3(sx * 0.28, 0.48, 0.24), 0.055, 0.08, mat_fab, false)
		# Mahogany under-arm + leg connection
		_add_mesh_cyl(root, Vector3(sx * 0.28, 0.3, 0.12), 0.016, 0.12, mat_md, true)
		_add_mesh_cyl(root, Vector3(sx * 0.28, 0.24, 0.18), 0.024, 0.03, mat_m, false)
	# Diamond dark buttons on back face
	for row in 4:
		var by := 0.66 + float(row) * 0.12
		var odd := row % 2 == 1
		var cols: Array = [-0.06, 0.06] if odd else [-0.1, 0.0, 0.1]
		for bx_v in cols:
			_add_mesh_cyl(root, Vector3(float(bx_v), by, 0.12), 0.01, 0.008, mat_fab_dd, false)
	# Turned legs + open H-stretcher (air under chair)
	for sx in [-1.0, 1.0]:
		_add_mesh_cyl(root, Vector3(sx * 0.22, 0.1, 0.18), 0.022, 0.18, mat_md, true)
		_add_mesh_cyl(root, Vector3(sx * 0.22, 0.018, 0.18), 0.032, 0.022, mat_m, false)
		_add_mesh_cyl(root, Vector3(sx * 0.2, 0.1, -0.14), 0.02, 0.18, mat_md, true)
		_add_mesh_cyl(root, Vector3(sx * 0.2, 0.018, -0.14), 0.03, 0.022, mat_m, false)
	_add_mesh_box(root, Vector3(0, 0.1, 0.02), Vector3(0.4, 0.012, 0.012), mat_m)
	_add_mesh_box(root, Vector3(-0.2, 0.1, 0.02), Vector3(0.012, 0.012, 0.28), mat_m)
	_add_mesh_box(root, Vector3(0.2, 0.1, 0.02), Vector3(0.012, 0.012, 0.28), mat_m)
	_add_contact_shadow(root, 0.48, 0.44)
	return root
...
static func _make_ottoman(prop: Dictionary) -> Node3D:
	...
	## Loop 239: square residual = green Minecraft cushion box mid-FOV
	## (morning_room_from_north / drawing_room_spawn). Soft pillow mass + roll edge.
	var root := Node3D.new()
	root.name = "Ottoman"
	...
	if style == 1:
		...
		# Loop 239: square chesterfield residual = green Minecraft cushion box mid-FOV
		# (morning_room_from_north / drawing_room_spawn). Soft continuous pillow mass —
		# roll-dominant perimeter, cylinder dome crown, slim plinth + bun feet only.
		# NO stacked body boxes / wedding-cake tiers.
		# Slim mahogany plinth (thin rail only — not deep brick base)
		_add_mesh_box(root, Vector3(0, 0.06, 0), Vector3(w * 0.86, 0.04, d * 0.82), mat_md)
		_add_mesh_box(root, Vector3(0, 0.09, 0), Vector3(w * 0.9, 0.022, d * 0.86), mat_m)
		# Continuous soft body via overlapping same-family cylinders (not box stack)
		_add_mesh_cyl(root, Vector3(0, 0.22, 0), w * 0.36, 0.22, mat_fab, true)
		_add_mesh_box(root, Vector3(0, 0.22, 0), Vector3(w * 0.78, 0.18, d * 0.72), mat_fab)
		# Soft dome crown — cylinder steps only (same fabric family)
		_add_mesh_cyl(root, Vector3(0, 0.36, 0), w * 0.32, 0.1, mat_fab_d, false)
		_add_mesh_cyl(root, Vector3(0, 0.42, 0), w * 0.24, 0.07, mat_fab_h, false)
		_add_mesh_cyl(root, Vector3(0, 0.46, 0), w * 0.14, 0.04, mat_fab_d, false)
		# Perimeter bolster rolls dominate silhouette (soft edge, not hard box corners)
		_add_mesh_cyl_rot(root, Vector3(0, 0.28, d * 0.34), 0.07, w * 0.78, mat_fab_h, Vector3(0, 0, PI * 0.5))
		_add_mesh_cyl_rot(root, Vector3(0, 0.28, -d * 0.34), 0.07, w * 0.78, mat_fab_h, Vector3(0, 0, PI * 0.5))
		_add_mesh_cyl_rot(root, Vector3(w * 0.36, 0.28, 0), 0.07, d * 0.68, mat_fab_h, Vector3(PI * 0.5, 0, 0))
		_add_mesh_cyl_rot(root, Vector3(-w * 0.36, 0.28, 0), 0.07, d * 0.68, mat_fab_h, Vector3(PI * 0.5, 0, 0))
		# Corner puffs (soft blobs, not box corners)
		for sx in [-1.0, 1.0]:
			for sz in [-1.0, 1.0]:
				_add_mesh_cyl(root, Vector3(sx * w * 0.32, 0.3, sz * d * 0.28), 0.065, 0.12, mat_fab, false)
		# Dark diamond tufts only (not bright squares)
		for row in 3:
			var by2 := 0.48
			var odd2 := row % 2 == 1
			var cols2: Array = [-0.1, 0.1] if odd2 else [-0.18, 0.0, 0.18]
			for bx_v in cols2:
				var bz_v := (float(row) - 1.0) * d * 0.16
				_add_mesh_cyl(root, Vector3(float(bx_v), by2, bz_v), 0.016, 0.012, mat_fab_dd, false)
		# Tiny fringe teeth under front/back roll only
		for i in 6:
			var fx := (float(i) / 5.0 - 0.5) * w * 0.72
			_add_mesh_box(root, Vector3(fx, 0.1, d * 0.38), Vector3(0.028, 0.05, 0.014), mat_fab_dd)
			_add_mesh_box(root, Vector3(fx, 0.1, -d * 0.38), Vector3(0.028, 0.05, 0.014), mat_fab_dd)
		# Brass nail row on front rail
		for i in 5:
			var t := float(i) / 4.0
			_add_mesh_cyl(root, Vector3(-w * 0.32 + t * w * 0.64, 0.12, d * 0.4), 0.009, 0.009, mat_br, false)
		# Bun feet
		for sx in [-1.0, 1.0]:
			for sz in [-1.0, 1.0]:
				_add_mesh_cyl(root, Vector3(sx * w * 0.3, 0.04, sz * d * 0.26), 0.032, 0.08, mat_m, true)
				_add_mesh_cyl(root, Vector3(sx * w * 0.3, 0.01, sz * d * 0.26), 0.04, 0.022, mat_md, false)
...
static func _make_hall_table(prop: Dictionary) -> Node3D:
	...
	if dress == 0:
		...
		# Loop 185/224: porcelain bud vase — warm ivory (not pale cream cake mid-FOV hall).
		var mat_vase := _solid_matte(Color(0.72, 0.66, 0.55), 0.5)
		var mat_vase_d := _solid_matte(Color(0.52, 0.46, 0.38), 0.55)
		var mat_blue := _solid_matte(Color(0.2, 0.3, 0.45), 0.4)
		...
		# Slim bottle: foot → belly → narrow neck (continuous taper, not tiers)
		_add_mesh_cyl(root, Vector3(0.32, ty + 0.015, 0), 0.04, 0.02, mat_vase_d, false)
		_add_mesh_cyl(root, Vector3(0.32, ty + 0.08, 0), 0.048, 0.1, mat_vase, false)
		_add_mesh_cyl(root, Vector3(0.32, ty + 0.1, 0), 0.05, 0.015, mat_blue, false)
		_add_mesh_cyl(root, Vector3(0.32, ty + 0.16, 0), 0.028, 0.07, mat_vase_d, false)
		_add_mesh_cyl(root, Vector3(0.32, ty + 0.21, 0), 0.035, 0.018, mat_vase, false)
		# Low irregular bloom cluster (not lid disc)
		_add_mesh_box(root, Vector3(0.32, ty + 0.24, 0.0), Vector3(0.07, 0.018, 0.055), mat_leaf)
		_add_mesh_cyl(root, Vector3(0.32, ty + 0.26, 0.01), 0.018, 0.014, mat_fl_r, false)
		_add_mesh_cyl(root, Vector3(0.35, ty + 0.255, -0.012), 0.014, 0.011, mat_fl_p, false)
		_add_mesh_cyl(root, Vector3(0.29, ty + 0.252, 0.018), 0.012, 0.01, mat_fl_y, false)
		_add_mesh_cyl(root, Vector3(0.33, ty + 0.27, 0.0), 0.01, 0.009, mat_fl_r, false)
...
static func _make_prep_table(prop: Dictionary) -> Node3D:
	...
	# Loop 219: lower shelf still-life — not anonymous tin box + crock nubs mid-FOV.
	# Loaf tin with crust peek, lidded stoneware, nested copper + bail, folded cloth.
	var mat_sh_oak := _solid_matte(Color(0.52, 0.4, 0.24), 0.82)
	var mat_sh_oak_d := _solid_matte(Color(0.38, 0.26, 0.14), 0.85)
	var mat_sh_tin := _solid_metal(Color(0.52, 0.48, 0.44), 0.42)
	var mat_sh_tin_l := _solid_metal(Color(0.62, 0.58, 0.52), 0.38)
	var mat_sh_crust := _solid_matte(Color(0.58, 0.38, 0.18), 0.9)
	var mat_sh_crumb := _solid_matte(Color(0.82, 0.7, 0.48), 0.94)
	var mat_sh_crock := _solid_matte(Color(0.72, 0.62, 0.5), 0.9)
	var mat_sh_crock_d := _solid_matte(Color(0.55, 0.45, 0.35), 0.9)
	var mat_sh_glaze := _solid_matte(Color(0.35, 0.42, 0.48), 0.55)
	var mat_sh_cop := _solid_metal(Color(0.7, 0.42, 0.18), 0.32)
	var mat_sh_cop_l := _solid_metal(Color(0.78, 0.5, 0.24), 0.28)
	var mat_sh_cop_d := _solid_metal(Color(0.55, 0.32, 0.14), 0.36)
	var mat_sh_cloth := _solid_matte(Color(0.68, 0.64, 0.54), 0.94)
	var mat_sh_cloth_d := _solid_matte(Color(0.58, 0.54, 0.44), 0.94)
	var mat_sh_jar := _solid_matte(Color(0.55, 0.28, 0.14), 0.88)
	var mat_sh_jar_l := _solid_matte(Color(0.65, 0.36, 0.18), 0.88)
	_add_mesh_box(root, Vector3(0, 0.28, 0), Vector3(width * 0.72, 0.03, 0.55), mat_sh_oak)
	# Plank edge lip (reads as real shelf, not floating slab)
	_add_mesh_box(root, Vector3(0, 0.305, 0.26), Vector3(width * 0.7, 0.02, 0.025), mat_sh_oak_d)
	# --- Loaf tin (metal walls + golden crust peek — not solid wood box) ---
	var lx := -width * 0.2
	_add_mesh_box(root, Vector3(lx, 0.34, 0.02), Vector3(0.3, 0.1, 0.18), mat_sh_tin)
	_add_mesh_box(root, Vector3(lx, 0.395, 0.02), Vector3(0.28, 0.018, 0.16), mat_sh_tin_l)
	# Inner crust loaf (slightly proud of tin rim)
	_add_mesh_box(root, Vector3(lx, 0.4, 0.02), Vector3(0.24, 0.05, 0.12), mat_sh_crust)
	_add_mesh_box(root, Vector3(lx, 0.43, 0.02), Vector3(0.2, 0.025, 0.09), mat_sh_crumb)
	# Score lines on crust
	_add_mesh_box(root, Vector3(lx - 0.04, 0.445, 0.02), Vector3(0.01, 0.008, 0.08), mat_sh_crust)
	_add_mesh_box(root, Vector3(lx + 0.04, 0.445, 0.02), Vector3(0.01, 0.008, 0.08), mat_sh_crust)
	# Tin end handles
	_add_mesh_box(root, Vector3(lx - 0.16, 0.37, 0.02), Vector3(0.03, 0.03, 0.08), mat_sh_tin_l)
	_add_mesh_box(root, Vector3(lx + 0.16, 0.37, 0.02), Vector3(0.03, 0.03, 0.08), mat_sh_tin_l)
	# --- Lidded stoneware crock with glaze band + knob ---
	var cx := width * 0.02
	_add_mesh_cyl(root, Vector3(cx, 0.38, 0.12), 0.065, 0.14, mat_sh_crock, false)
	_add_mesh_cyl(root, Vector3(cx, 0.4, 0.12), 0.07, 0.025, mat_sh_glaze, false)
	_add_mesh_cyl(root, Vector3(cx, 0.46, 0.12), 0.055, 0.03, mat_sh_crock_d, false)
	_add_mesh_cyl(root, Vector3(cx, 0.48, 0.12), 0.035, 0.02, mat_sh_crock, false)
	_add_mesh_cyl(root, Vector3(cx, 0.495, 0.12), 0.018, 0.02, mat_sh_crock_d, false)
	# Side ear handle
	_add_mesh_box(root, Vector3(cx + 0.07, 0.4, 0.12), Vector3(0.03, 0.05, 0.025), mat_sh_crock_d)
	# --- Nested copper pans (upturned + bail) ---
	var px := width * 0.2
	_add_mesh_cyl(root, Vector3(px, 0.34, -0.1), 0.1, 0.04, mat_sh_cop, false)
	_add_mesh_cyl(root, Vector3(px, 0.37, -0.1), 0.11, 0.02, mat_sh_cop_l, false)
	_add_mesh_cyl(root, Vector3(px, 0.39, -0.1), 0.075, 0.035, mat_sh_cop_d, false)
	_add_mesh_cyl(root, Vector3(px, 0.42, -0.1), 0.08, 0.016, mat_sh_cop_l, false)
	# Horizontal bail handle across top pan
	_add_mesh_cyl_rot(root, Vector3(px, 0.445, -0.1), 0.01, 0.14, mat_sh_cop_d, Vector3(0, 0, PI * 0.5))
	_add_mesh_cyl(root, Vector3(px - 0.07, 0.43, -0.1), 0.012, 0.025, mat_sh_cop, false)
	_add_mesh_cyl(root, Vector3(px + 0.07, 0.43, -0.1), 0.012, 0.025, mat_sh_cop, false)
	# --- Folded linen + small jam jar (fills empty shelf span) ---
	_add_mesh_box(root, Vector3(-width * 0.02, 0.31, -0.15), Vector3(0.16, 0.018, 0.12), mat_sh_cloth)
	_add_mesh_box(root, Vector3(0.0, 0.325, -0.13), Vector3(0.12, 0.014, 0.08), mat_sh_cloth_d)
	_add_mesh_cyl(root, Vector3(width * 0.12, 0.36, 0.14), 0.035, 0.08, mat_sh_jar, false)
	_add_mesh_cyl(root, Vector3(width * 0.12, 0.41, 0.14), 0.028, 0.025, mat_sh_jar_l, false)
	_add_mesh_cyl(root, Vector3(width * 0.12, 0.425, 0.14), 0.02, 0.015, mat_sh_crock_d, false)
	match kit:
		0:
		...
		1:
			# Loop 215: baking still-life — pie tin + fluted rim, plump dough mound,
			# flour-dusted board, egg crock (not flat dough slab mid-FOV).
			var mat_b_board := _solid_matte(Color(0.64, 0.52, 0.34), 0.85)
			var mat_b_board_d := _solid_matte(Color(0.5, 0.38, 0.22), 0.88)
			var mat_b_board_e := _solid_matte(Color(0.42, 0.3, 0.16), 0.82)
			var mat_dough := _solid_matte(Color(0.84, 0.76, 0.58), 0.95)
			var mat_dough_d := _solid_matte(Color(0.76, 0.68, 0.5), 0.95)
			var mat_pie := _solid_matte(Color(0.55, 0.22, 0.18), 0.85)
			var mat_pie_l := _solid_matte(Color(0.65, 0.3, 0.2), 0.85)
			var mat_crust_b := _solid_matte(Color(0.78, 0.58, 0.36), 0.88)
			var mat_tin := _solid_metal(Color(0.55, 0.52, 0.48), 0.4)
			...
			var mat_butter := _solid_matte(Color(0.9, 0.84, 0.62), 0.94)
			var mat_egg := _solid_matte(Color(0.9, 0.86, 0.72), 0.9)
			var mat_flour := _solid_matte(Color(0.82, 0.76, 0.58), 0.97)
			var mat_cloth_b := _solid_matte(Color(0.7, 0.72, 0.68), 0.92)
			# Thick board + flour dust
			_add_mesh_box(root, Vector3(-0.35, 0.87, 0.0), Vector3(0.62, 0.04, 0.42), mat_b_board_d)
			_add_mesh_box(root, Vector3(-0.35, 0.892, 0.0), Vector3(0.58, 0.012, 0.38), mat_b_board)
			_add_mesh_box(root, Vector3(-0.35, 0.9, 0.18), Vector3(0.55, 0.006, 0.025), mat_b_board_e)
			_add_mesh_box(root, Vector3(-0.35, 0.898, 0.0), Vector3(0.35, 0.005, 0.22), mat_flour)
			# Plump dough mound (not flat slab)
			_add_mesh_cyl(root, Vector3(-0.4, 0.92, 0.0), 0.1, 0.055, mat_dough, false)
			_add_mesh_cyl(root, Vector3(-0.4, 0.95, 0.0), 0.085, 0.04, mat_dough_d, false)
			_add_mesh_cyl(root, Vector3(-0.38, 0.97, 0.02), 0.06, 0.03, mat_dough, false)
			# Pie in tin with fluted rim + lattice hint
			_add_mesh_cyl(root, Vector3(0.12, 0.895, 0.08), 0.13, 0.035, mat_tin, false)
			_add_mesh_cyl(root, Vector3(0.12, 0.92, 0.08), 0.14, 0.02, mat_tin, false)
			_add_mesh_cyl(root, Vector3(0.12, 0.925, 0.08), 0.11, 0.03, mat_pie, false)
			_add_mesh_cyl(root, Vector3(0.12, 0.945, 0.08), 0.095, 0.02, mat_pie_l, false)
			_add_mesh_box(root, Vector3(0.12, 0.955, 0.08), Vector3(0.14, 0.008, 0.02), mat_crust_b)
			_add_mesh_box(root, Vector3(0.12, 0.955, 0.08), Vector3(0.02, 0.008, 0.14), mat_crust_b)
			# Rolling pin
			_add_mesh_cyl_rot(root, Vector3(0.35, 0.91, -0.14), 0.03, 0.4, mat_pin, Vector3(0, 0.25, PI * 0.5))
			_add_mesh_cyl_rot(root, Vector3(0.14, 0.91, -0.18), 0.018, 0.055, mat_pin_h, Vector3(0, 0.25, PI * 0.5))
			_add_mesh_cyl_rot(root, Vector3(0.56, 0.91, -0.1), 0.018, 0.055, mat_pin_h, Vector3(0, 0.25, PI * 0.5))
			# Mixing bowl + flour cloud
			_add_mesh_cyl(root, Vector3(0.5, 0.92, 0.14), 0.12, 0.09, mat_bowl, false)
			_add_mesh_cyl(root, Vector3(0.5, 0.97, 0.14), 0.13, 0.02, mat_bowl_r, false)
			_add_mesh_cyl(root, Vector3(0.5, 0.95, 0.14), 0.08, 0.02, mat_flour, false)
			# Butter brick + eggs in crock
			_add_mesh_box(root, Vector3(-0.08, 0.88, 0.22), Vector3(0.12, 0.035, 0.08), mat_butter)
			_add_mesh_cyl(root, Vector3(-0.08, 0.905, 0.22), 0.03, 0.025, mat_butter, false)
			_add_mesh_cyl(root, Vector3(0.22, 0.9, 0.2), 0.055, 0.05, mat_bowl_r, false)
			_add_mesh_cyl(root, Vector3(0.2, 0.94, 0.2), 0.022, 0.03, mat_egg, false)
			_add_mesh_cyl(root, Vector3(0.25, 0.935, 0.18), 0.02, 0.028, mat_egg, false)
			_add_mesh_box(root, Vector3(-0.55, 0.875, 0.18), Vector3(0.16, 0.014, 0.12), mat_cloth_b)
		2:
			# Loop 215: market veg still-life — oval willow trug of produce, thick chop
			# block with cleaver, onion/leek/cabbage mass (not sparse nubs mid-FOV).
			var mat_wick := _solid_matte(Color(0.48, 0.34, 0.18), 0.92)
			var mat_wick_d := _solid_matte(Color(0.36, 0.24, 0.12), 0.94)
			var mat_wick_l := _solid_matte(Color(0.55, 0.4, 0.22), 0.9)
			var mat_chop := _solid_matte(Color(0.55, 0.42, 0.26), 0.82)
			var mat_chop_d := _solid_matte(Color(0.42, 0.3, 0.16), 0.85)
			var mat_chop_e := _solid_matte(Color(0.36, 0.24, 0.12), 0.8)
			var mat_cleaver := _solid_metal(Color(0.55, 0.55, 0.58), 0.38)
			var mat_cleaver_h := _solid_matte(Color(0.3, 0.18, 0.1), 0.75)
			var mat_crock_v := _solid_matte(Color(0.72, 0.64, 0.52), 0.9)
			var mat_crock_vd := _solid_matte(Color(0.58, 0.5, 0.4), 0.9)
			var mat_cloth_v := _solid_matte(Color(0.68, 0.7, 0.62), 0.92)
			var mat_carrot := _solid_matte(Color(0.85, 0.48, 0.16), 0.88)
			var mat_green_a := _solid_matte(Color(0.24, 0.4, 0.14), 0.9)
			var mat_green_b := _solid_matte(Color(0.3, 0.44, 0.18), 0.9)
			var mat_green_c := _solid_matte(Color(0.2, 0.34, 0.12), 0.9)
			var mat_onion := _solid_matte(Color(0.78, 0.62, 0.32), 0.88)
			var mat_onion_d := _solid_matte(Color(0.65, 0.5, 0.24), 0.88)
			var mat_leek := _solid_matte(Color(0.55, 0.7, 0.4), 0.9)
			var mat_leek_w := _solid_matte(Color(0.82, 0.85, 0.72), 0.92)
			var mat_apple_v := _solid_matte(Color(0.55, 0.16, 0.12), 0.9)
			var mat_apple_g := _solid_matte(Color(0.45, 0.5, 0.2), 0.9)
			# Oval willow trug (multi-cyl body — not single disc)
			_add_mesh_cyl(root, Vector3(-0.4, 0.91, 0.0), 0.14, 0.09, mat_wick, false)
			_add_mesh_cyl(root, Vector3(-0.32, 0.91, 0.0), 0.12, 0.09, mat_wick, false)
			_add_mesh_cyl(root, Vector3(-0.48, 0.91, 0.0), 0.12, 0.09, mat_wick, false)
			_add_mesh_cyl(root, Vector3(-0.4, 0.88, 0.0), 0.15, 0.03, mat_wick_d, false)
			_add_mesh_cyl(root, Vector3(-0.4, 0.96, 0.0), 0.17, 0.025, mat_wick_l, false)
			# Weave ribs
			for bi in 3:
				var by := 0.89 + float(bi) * 0.025
				_add_mesh_cyl(root, Vector3(-0.4, by, 0.0), 0.155, 0.01, mat_wick_d, false)
			# Handles
			_add_mesh_box(root, Vector3(-0.4, 0.95, 0.14), Vector3(0.22, 0.03, 0.02), mat_wick_d)
			_add_mesh_box(root, Vector3(-0.4, 0.95, -0.14), Vector3(0.22, 0.03, 0.02), mat_wick_d)
			# Cabbage mound + leaves
			_add_mesh_cyl(root, Vector3(-0.4, 1.0, 0.0), 0.09, 0.07, mat_green_a, false)
			_add_mesh_cyl(root, Vector3(-0.36, 1.04, 0.04), 0.06, 0.04, mat_green_b, false)
			_add_mesh_cyl(root, Vector3(-0.45, 1.03, -0.03), 0.055, 0.035, mat_green_c, false)
			_add_mesh_box(root, Vector3(-0.32, 1.0, 0.06), Vector3(0.08, 0.015, 0.05), mat_green_b)
			# Carrots protruding
			_add_mesh_cyl_rot(root, Vector3(-0.28, 0.99, -0.05), 0.016, 0.12, mat_carrot, Vector3(0, 0.2, PI * 0.4))
			_add_mesh_cyl_rot(root, Vector3(-0.5, 0.99, 0.04), 0.014, 0.1, mat_carrot, Vector3(0, -0.3, -PI * 0.35))
			_add_mesh_box(root, Vector3(-0.22, 1.02, -0.05), Vector3(0.04, 0.02, 0.03), mat_green_a)
			# Onions on table edge
			_add_mesh_cyl(root, Vector3(-0.15, 0.9, 0.18), 0.04, 0.04, mat_onion, false)
			_add_mesh_cyl(root, Vector3(-0.15, 0.925, 0.18), 0.035, 0.025, mat_onion_d, false)
			_add_mesh_cyl(root, Vector3(-0.08, 0.895, 0.2), 0.035, 0.035, mat_onion_d, false)
			# Leeks bundle
			_add_mesh_cyl_rot(root, Vector3(0.05, 0.91, 0.18), 0.02, 0.22, mat_leek_w, Vector3(0, 0.4, PI * 0.5))
			_add_mesh_cyl_rot(root, Vector3(0.05, 0.925, 0.16), 0.018, 0.14, mat_leek, Vector3(0, 0.4, PI * 0.5))
			_add_mesh_cyl_rot(root, Vector3(0.08, 0.91, 0.2), 0.016, 0.18, mat_leek_w, Vector3(0, 0.35, PI * 0.5))
			# Thick chop block + cleaver
			_add_mesh_box(root, Vector3(0.32, 0.88, -0.05), Vector3(0.45, 0.1, 0.32), mat_chop_d)
			_add_mesh_box(root, Vector3(0.32, 0.94, -0.05), Vector3(0.42, 0.04, 0.28), mat_chop)
			_add_mesh_box(root, Vector3(0.32, 0.96, -0.05), Vector3(0.38, 0.01, 0.24), mat_chop_e)
			# Cleaver with thick blade + wood handle
			_add_mesh_box(root, Vector3(0.38, 0.98, 0.02), Vector3(0.18, 0.02, 0.05), mat_cleaver)
			_add_mesh_box(root, Vector3(0.42, 0.99, 0.02), Vector3(0.1, 0.01, 0.035), mat_cleaver)
			_add_mesh_cyl_rot(root, Vector3(0.24, 0.98, 0.02), 0.015, 0.09, mat_cleaver_h, Vector3(0, 0, PI * 0.5))
			# Cream crock + apples
			_add_mesh_cyl(root, Vector3(0.55, 0.93, 0.14), 0.05, 0.1, mat_crock_v, false)
			_add_mesh_cyl(root, Vector3(0.55, 0.93, 0.14), 0.055, 0.02, mat_crock_vd, false)
			_add_mesh_cyl(root, Vector3(0.55, 1.0, 0.14), 0.038, 0.025, mat_crock_vd, false)
			_add_mesh_cyl(root, Vector3(0.48, 0.9, -0.16), 0.04, 0.035, mat_apple_v, false)
			_add_mesh_cyl(root, Vector3(0.48, 0.925, -0.16), 0.045, 0.03, mat_apple_v, false)
			_add_mesh_cyl(root, Vector3(0.55, 0.895, -0.12), 0.035, 0.03, mat_apple_g, false)
			_add_mesh_cyl(root, Vector3(0.55, 0.92, -0.12), 0.04, 0.025, mat_apple_g, false)
			_add_mesh_box(root, Vector3(0.1, 0.875, 0.2), Vector3(0.16, 0.012, 0.1), mat_cloth_v)
		_:
			# Loop 215: scullery kit — mortar bowl + pestle head, colander with holes,
			# herb bunch, spice jar (not flat herb slab mid-FOV).
			var mat_herb_b := _solid_matte(Color(0.58, 0.46, 0.3), 0.85)
			var mat_herb_d := _solid_matte(Color(0.45, 0.34, 0.2), 0.88)
			var mat_leaf_a := _solid_matte(Color(0.26, 0.4, 0.16), 0.92)
			var mat_leaf_b := _solid_matte(Color(0.32, 0.4, 0.18), 0.92)
			var mat_leaf_c := _solid_matte(Color(0.22, 0.36, 0.14), 0.92)
			var mat_stone_m := _solid_matte(Color(0.55, 0.52, 0.46), 0.88)
			var mat_stone_r := _solid_matte(Color(0.45, 0.42, 0.38), 0.88)
			var mat_pestle := _solid_matte(Color(0.4, 0.28, 0.14), 0.8)
			var mat_pestle_h := _solid_matte(Color(0.5, 0.36, 0.2), 0.78)
			var mat_col := _solid_metal(Color(0.7, 0.42, 0.18), 0.32)
			var mat_col_l := _solid_metal(Color(0.78, 0.5, 0.24), 0.28)
			var mat_hole := _solid_matte(Color(0.12, 0.1, 0.08), 0.9)
			...
			var mat_spice := _solid_matte(Color(0.55, 0.35, 0.15), 0.9)
			# Herb board with irregular bunch mass
			_add_mesh_box(root, Vector3(-0.35, 0.875, 0.05), Vector3(0.5, 0.03, 0.34), mat_herb_d)
			_add_mesh_box(root, Vector3(-0.35, 0.892, 0.05), Vector3(0.46, 0.012, 0.3), mat_herb_b)
			# Irregular herb pile (overlapping pads)
			_add_mesh_box(root, Vector3(-0.42, 0.91, 0.08), Vector3(0.2, 0.035, 0.12), mat_leaf_a)
			_add_mesh_box(root, Vector3(-0.32, 0.92, 0.0), Vector3(0.16, 0.03, 0.14), mat_leaf_b)
			_add_mesh_box(root, Vector3(-0.28, 0.915, 0.1), Vector3(0.14, 0.025, 0.1), mat_leaf_c)
			_add_mesh_cyl(root, Vector3(-0.38, 0.925, 0.04), 0.05, 0.03, mat_leaf_a, false)
			_add_mesh_cyl(root, Vector3(-0.3, 0.93, 0.06), 0.04, 0.025, mat_leaf_b, false)
			# Stone mortar (bowl) + pestle with bulb head
			_add_mesh_cyl(root, Vector3(0.12, 0.92, 0.05), 0.1, 0.1, mat_stone_m, false)
			_add_mesh_cyl(root, Vector3(0.12, 0.98, 0.05), 0.11, 0.03, mat_stone_r, false)
			_add_mesh_cyl(root, Vector3(0.12, 0.95, 0.05), 0.07, 0.02, mat_spice, false)
			_add_mesh_cyl(root, Vector3(0.2, 1.02, 0.08), 0.022, 0.18, mat_pestle, false)
			_add_mesh_cyl(root, Vector3(0.2, 0.95, 0.08), 0.04, 0.05, mat_pestle_h, false)
			# Copper colander with rim + drain holes
			_add_mesh_cyl(root, Vector3(0.5, 0.9, -0.1), 0.13, 0.055, mat_col, false)
			_add_mesh_cyl(root, Vector3(0.5, 0.94, -0.1), 0.14, 0.02, mat_col_l, false)
			_add_mesh_cyl(root, Vector3(0.5, 0.88, -0.1), 0.08, 0.02, mat_col, false)
			for hi in 6:
				var ha: float = float(hi) * TAU / 6.0
				_add_mesh_cyl(root, Vector3(0.5 + cos(ha) * 0.07, 0.91, -0.1 + sin(ha) * 0.07), 0.012, 0.01, mat_hole, false)
			# Wooden spoon + copper ladle
			_add_mesh_box(root, Vector3(-0.1, 0.89, -0.2), Vector3(0.3, 0.015, 0.03), mat_spoon)
			_add_mesh_cyl(root, Vector3(-0.28, 0.89, -0.2), 0.03, 0.022, mat_spoon, false)
			_add_mesh_box(root, Vector3(0.25, 0.89, 0.2), Vector3(0.28, 0.015, 0.025), mat_ladle)
			_add_mesh_cyl(root, Vector3(0.08, 0.89, 0.2), 0.04, 0.03, mat_ladle_c, false)
			# Spice jar + cloth
			_add_mesh_cyl(root, Vector3(-0.55, 0.92, 0.18), 0.045, 0.1, mat_jar, false)
			_add_mesh_cyl(root, Vector3(-0.55, 0.98, 0.18), 0.035, 0.03, mat_jar_d, false)
			_add_mesh_cyl(root, Vector3(-0.55, 0.94, 0.18), 0.048, 0.015, mat_spice, false)
			_add_mesh_box(root, Vector3(0.5, 0.875, 0.18), Vector3(0.16, 0.014, 0.12), mat_cloth_s)
...
static func _make_floor_path(prop: Dictionary) -> Node3D:
	...
	if surface == "stone":
		...
		_add_mesh_box(root, Vector3(0, 0.008, 0), Vector3(width * 0.95, 0.01, length * 0.9), mat_grit)
		for side in [-1.0, 1.0]:
			for ci in 9:
				var cz := -length * 0.44 + float(ci) * (length * 0.88 / 8.0)
				var cw := 0.14 + float((ci + seed0) % 4) * 0.04
				var ch := 0.032 + float((ci + seed0) % 3) * 0.01
				var cx: float = float(side) * (width * 0.55 + float(ci % 3) * 0.025)
				var mat_c: StandardMaterial3D = mat_grit_d if ci % 2 == 0 else mat_peb
				_add_mesh_box(root, Vector3(cx, ch * 0.5, cz), Vector3(cw, ch, 0.32 + float(ci % 4) * 0.1), mat_c)
				if ci % 2 == 0:
					_add_mesh_box(root, Vector3(cx, 0.012, cz + 0.1), Vector3(0.06, 0.01, 0.1), mat_moss)
	return root
...
static func _make_urn(prop: Dictionary) -> Node3D:
	...
	## Loop 226: darker sandstone under glass wash (pale white residual conservatory).
	var root := Node3D.new()
	root.name = "Urn"
	var seed0: int = int(prop.get("seed", 0))
	var scale: float = float(prop.get("scale", 1.0))
	# Dark warm sandstone — survives glass wash (pale cream → white mid-FOV)
	var body_c := Color(0.32, 0.26, 0.2) if seed0 % 2 == 0 else Color(0.28, 0.24, 0.2)
	var mat := _solid_matte(body_c, 0.94)
	var mat_d := _solid_matte(body_c.darkened(0.12), 0.94)
	var mat_l := _solid_matte(body_c.lightened(0.06), 0.92)
	var mat_soil := _solid_matte(Color(0.12, 0.08, 0.04), 0.95)
	var mat_la := _solid_matte(Color(0.16, 0.3, 0.1), 0.9)
	var mat_lb := _solid_matte(Color(0.1, 0.22, 0.06), 0.9)
	# Round base disc + plinth ring
	_add_mesh_cyl(root, Vector3(0, 0.03 * scale, 0), 0.22 * scale, 0.06 * scale, mat_d, true)
	_add_mesh_cyl(root, Vector3(0, 0.08 * scale, 0), 0.17 * scale, 0.035 * scale, mat, false)
	# Turned pedestal stem (narrow)
	_add_mesh_cyl(root, Vector3(0, 0.18 * scale, 0), 0.065 * scale, 0.14 * scale, mat, true)
	_add_mesh_cyl(root, Vector3(0, 0.3 * scale, 0), 0.09 * scale, 0.08 * scale, mat_l, true)
	_add_mesh_cyl(root, Vector3(0, 0.38 * scale, 0), 0.055 * scale, 0.08 * scale, mat_d, false)
	# Cup/bowl (wide mouth)
	_add_mesh_cyl(root, Vector3(0, 0.5 * scale, 0), 0.15 * scale, 0.14 * scale, mat, true)
	_add_mesh_cyl(root, Vector3(0, 0.58 * scale, 0), 0.21 * scale, 0.1 * scale, mat_l, true)
	# Rolled rim
	_add_mesh_cyl(root, Vector3(0, 0.64 * scale, 0), 0.23 * scale, 0.035 * scale, mat_d, false)
	# Inner soil
	_add_mesh_cyl(root, Vector3(0, 0.62 * scale, 0), 0.14 * scale, 0.025 * scale, mat_soil, false)
	# Loop handles
	for sx in [-1.0, 1.0]:
		_add_mesh_box(root, Vector3(sx * 0.21 * scale, 0.54 * scale, 0), Vector3(0.03 * scale, 0.1 * scale, 0.04 * scale), mat_d)
		_add_mesh_box(root, Vector3(sx * 0.23 * scale, 0.6 * scale, 0), Vector3(0.04 * scale, 0.025 * scale, 0.04 * scale), mat)
		_add_mesh_box(root, Vector3(sx * 0.19 * scale, 0.48 * scale, 0), Vector3(0.035 * scale, 0.025 * scale, 0.04 * scale), mat)
	# Trailing ivy from rim
	for i in 4:
		var ang := float(i) * TAU / 4.0 + float(seed0) * 0.35
		var lx: float = cos(ang) * 0.15 * scale
		var lz: float = sin(ang) * 0.15 * scale
		_add_mesh_box(
			root,
			Vector3(lx, 0.58 * scale, lz),
			Vector3(0.055 * scale, 0.014 * scale, 0.035 * scale),
			mat_la if i % 2 == 0 else mat_lb
		)
		_add_mesh_box(
			root,
			Vector3(lx * 1.15, 0.48 * scale, lz * 1.15),
			Vector3(0.04 * scale, 0.01 * scale, 0.03 * scale),
...
static func _make_oil_lamp(prop: Dictionary) -> Node3D:
	...
	var mat_br := _solid_metal(Color(0.72, 0.56, 0.28), 0.3)
	var mat_br_d := _solid_metal(Color(0.55, 0.42, 0.2), 0.35)
	var mat_br_l := _solid_metal(Color(0.8, 0.64, 0.34), 0.28)
	var mat_font := _solid_metal(Color(0.5, 0.36, 0.18), 0.4)
...
static func _make_machine(prop: Dictionary) -> Node3D:
	...
	match style:
		0:
			# Loop 216: open induction coil stand — not dense copper fridge stack mid-FOV.
			# Oak plinth + four slender posts + 3 airy coils + amber terminal.
			var col_h: float = maxf(height * 0.62, 1.2)
			# Oak furniture plinth (not iron slab base)
			_add_mesh_box(root, Vector3(0, 0.08, 0), Vector3(1.0, 0.14, 0.9), mat_oak_sd)
			_add_mesh_box(root, Vector3(0, 0.18, 0), Vector3(0.9, 0.06, 0.8), mat_oak_s)
			_add_mesh_box(root, Vector3(0, 0.22, 0), Vector3(0.72, 0.03, 0.64), mat_br_sd)
			var col_mid: float = 0.38 + col_h * 0.42
			# Central brass mast (thin)
			_add_mesh_cyl(root, Vector3(0, col_mid, 0), 0.04, col_h * 0.8, mat_br_sd, true)
			_add_mesh_cyl(root, Vector3(0, 0.32, 0), 0.06, 0.06, mat_br_s, false)
			# Four slender oak posts (open cage — air on all sides)
			for sx in [-1.0, 1.0]:
				for sz in [-1.0, 1.0]:
					_add_mesh_cyl(root, Vector3(sx * 0.32, 0.28, sz * 0.32), 0.045, 0.08, mat_oak_sd, false)
					_add_mesh_cyl(root, Vector3(sx * 0.32, col_mid, sz * 0.32), 0.035, col_h * 0.75, mat_oak_s, true)
					_add_mesh_cyl(root, Vector3(sx * 0.32, 0.35 + col_h * 0.78, sz * 0.32), 0.04, 0.05, mat_br_s, false)
			# Slim top ring (not solid lid)
			_add_mesh_box(root, Vector3(0, 0.38 + col_h * 0.78, 0), Vector3(0.7, 0.04, 0.7), mat_oak_s)
			_add_mesh_box(root, Vector3(0, 0.42 + col_h * 0.78, 0), Vector3(0.55, 0.025, 0.55), mat_br_sd)
			# THREE copper coils with large air gaps (not 4 dense fridge shelves)
			for i in 3:
				var y: float = 0.48 + float(i) * (col_h * 0.26)
				var r: float = 0.36 - float(i) * 0.04
				var mat_coil := mat_cop_s if i % 2 == 0 else mat_cop_sd
				var mat_coil_l := mat_cop_sl if i % 2 == 0 else mat_cop_s
				# Thin coil ring
				_add_mesh_cyl(root, Vector3(0, y, 0), r, 0.04, mat_coil, false)
				_add_mesh_cyl(root, Vector3(0, y + 0.015, 0), r * 0.88, 0.016, mat_coil_l, false)
				# Hollow centre (air through — not solid oak disc)
				_add_mesh_cyl(root, Vector3(0, y, 0), 0.06, 0.03, mat_br_sd, false)
				# Sparse winding studs (not dense 10-box belt)
				for s in 6:
					var ang: float = float(s) * (TAU / 6.0) + float(i) * 0.25
					_add_mesh_box(
						root,
						Vector3(cos(ang) * r * 0.95, y, sin(ang) * r * 0.95),
						Vector3(0.035, 0.025, 0.04),
						mat_coil_l
					)
				# One brass terminal post on rim
				var side := 1.0 if i % 2 == 0 else -1.0
				_add_mesh_cyl(root, Vector3(side * r * 0.9, y, 0), 0.025, 0.06, mat_br_s, false)
			top_y = 0.45 + col_h * 0.85
			# Amber terminal crown (compact — not cyan hat)
			...
			_add_mesh_cyl(root, Vector3(0, top_y, 0), 0.14, 0.04, mat_br0, false)
			_add_mesh_cyl(root, Vector3(0, top_y + 0.05, 0), 0.07, 0.07, mat_br0d, false)
			_add_mesh_cyl(root, Vector3(0, top_y + 0.12, 0), 0.065, 0.09, mat_amber, false)
			_add_mesh_cyl(root, Vector3(0, top_y + 0.18, 0), 0.045, 0.025, mat_br0, false)
			for ri in 4:
				var ra := float(ri) * TAU / 4.0
				_add_mesh_box(root, Vector3(cos(ra) * 0.09, top_y + 0.07, sin(ra) * 0.09), Vector3(0.014, 0.09, 0.014), mat_br0d)
			# Side gauge box + feed pipe
			_add_mesh_box(root, Vector3(0.5, 0.5, 0.08), Vector3(0.22, 0.35, 0.25), mat_oak_sd)
			for gi in 2:
				_add_mesh_cyl(root, Vector3(0.5, 0.55 + float(gi) * 0.12, 0.2), 0.045, 0.02, mat_br_s, false)
				_add_mesh_cyl(root, Vector3(0.5, 0.56 + float(gi) * 0.12, 0.22), 0.032, 0.01, mat_gauge, false)
			_add_mesh_cyl(root, Vector3(-0.48, 0.8, 0.0), 0.03, 0.6, mat_cop_s, false)
			_add_mesh_cyl(root, Vector3(-0.48, 1.15, 0.12), 0.04, 0.035, mat_br_s, false)
		1:
...
static func _make_umbrella_stand(prop: Dictionary) -> Node3D:
	...
	## Loop 224: not pale milk-churn / anonymous cylinder mid-FOV (entrance_hall_corner).
	## Urn taper + warm stoneware or oak, crook sticks with fabric canopy hints.
	var root := Node3D.new()
	...
	var mat_canopy := _solid_matte(Color(0.28, 0.12, 0.1) if seed0 % 2 == 0 else Color(0.12, 0.16, 0.22), 0.9)
	if seed0 % 2 == 0:
		var mat_w := _solid_matte(Color(0.32, 0.2, 0.11), 0.72)
		var mat_wd := _solid_matte(Color(0.22, 0.13, 0.07), 0.75)
		var mat_wl := _solid_matte(Color(0.4, 0.26, 0.14), 0.7)
		# Turned oak urn: foot disc + tapered belly + brass rolled rim
		_add_mesh_cyl(root, Vector3(0, 0.03, 0), 0.18, 0.05, mat_wd, true)
		_add_mesh_cyl(root, Vector3(0, 0.1, 0), 0.12, 0.08, mat_w, false)
		_add_mesh_cyl(root, Vector3(0, 0.28, 0), 0.15, 0.28, mat_wl, false)
		_add_mesh_cyl(root, Vector3(0, 0.42, 0), 0.13, 0.12, mat_w, false)
		_add_mesh_cyl(root, Vector3(0, 0.52, 0), 0.16, 0.04, mat_br, false)
		_add_mesh_cyl(root, Vector3(0, 0.56, 0), 0.14, 0.025, mat_br_d, false)
		_add_mesh_cyl(root, Vector3(0, 0.58, 0), 0.12, 0.015, mat_br_l, false)
		# Inner dark well
		_add_mesh_cyl(root, Vector3(0, 0.35, 0), 0.09, 0.26, mat_wd, false)
	else:
		# Warm stoneware (not pale cream milk-churn) + blue glaze band
		var mat_cer := _solid_matte(Color(0.62, 0.52, 0.4), 0.88)
		var mat_cer_d := _solid_matte(Color(0.48, 0.4, 0.3), 0.9)
		var mat_cer_l := _solid_matte(Color(0.7, 0.58, 0.44), 0.86)
		var mat_glaze := _solid_matte(Color(0.28, 0.36, 0.48), 0.55)
		var mat_clay := _solid_matte(Color(0.5, 0.34, 0.2), 0.88)
		_add_mesh_cyl(root, Vector3(0, 0.03, 0), 0.18, 0.05, mat_clay, true)
		_add_mesh_cyl(root, Vector3(0, 0.12, 0), 0.11, 0.1, mat_cer_d, false)
		_add_mesh_cyl(root, Vector3(0, 0.3, 0), 0.155, 0.28, mat_cer, false)
		_add_mesh_cyl(root, Vector3(0, 0.36, 0), 0.16, 0.06, mat_glaze, false)
		_add_mesh_cyl(root, Vector3(0, 0.48, 0), 0.13, 0.1, mat_cer_l, false)
		_add_mesh_cyl(root, Vector3(0, 0.55, 0), 0.165, 0.035, mat_br, false)
		_add_mesh_cyl(root, Vector3(0, 0.58, 0), 0.14, 0.02, mat_br_d, false)
		_add_mesh_cyl(root, Vector3(0, 0.35, 0), 0.1, 0.26, mat_cer_d, false)
	# Umbrellas / sticks with crook handles + small canopy mass (identity)
	var n := 3 + seed0 % 2
	for i in n:
		var ang := float(i) * 0.95 + float(seed0) * 0.25
		var ox := cos(ang) * 0.055
		var oz := sin(ang) * 0.045
		var h := 0.58 + float((i + seed0) % 3) * 0.1
		var smat: Material = mat_stick if i % 2 == 0 else mat_stick_b
		_add_mesh_cyl(root, Vector3(ox, 0.58 + h * 0.42, oz), 0.014 + float(i) * 0.002, h, smat, false)
		# Crook handle
		_add_mesh_box(root, Vector3(ox + 0.045, 0.58 + h * 0.88, oz), Vector3(0.09, 0.022, 0.022), mat_handle)
		_add_mesh_cyl(root, Vector3(ox + 0.08, 0.58 + h * 0.82, oz), 0.018, 0.05, mat_handle, false)
		# Furled canopy blob near mid-stick (not bare pole)
		if i % 2 == 0:
			_add_mesh_cyl(root, Vector3(ox + 0.02, 0.75 + float(i) * 0.05, oz), 0.035, 0.08, mat_canopy, false)
...
static func _make_chandelier(_prop: Dictionary) -> Node3D:
	...
	## Loop 224: brass-dominant mid-FOV — short open bells (not cream ice-cream cone cluster).
	var root := Node3D.new()
	...
	var mat_rose := _solid_matte(Color(0.78, 0.72, 0.62), 0.85)
	# More transparent amber glass — brass reads first, not cream mass
	var mat_shade := StandardMaterial3D.new()
	mat_shade.albedo_color = Color(0.85, 0.68, 0.38, 0.38)
	mat_shade.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	mat_shade.metallic = 0.0
	mat_shade.roughness = 0.3
	mat_shade.emission_enabled = true
	mat_shade.emission = Color(1.0, 0.75, 0.4)
	mat_shade.emission_energy_multiplier = 0.7
	mat_shade.shading_mode = BaseMaterial3D.SHADING_MODE_PER_PIXEL
	var mat_shade_i := StandardMaterial3D.new()
	mat_shade_i.albedo_color = Color(1.0, 0.85, 0.5, 0.28)
	mat_shade_i.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	mat_shade_i.metallic = 0.0
	mat_shade_i.roughness = 0.2
	mat_shade_i.emission_enabled = true
	mat_shade_i.emission = Color(1.0, 0.82, 0.48)
	mat_shade_i.emission_energy_multiplier = 1.1
	mat_shade_i.shading_mode = BaseMaterial3D.SHADING_MODE_PER_PIXEL
	var mat_crystal := _solid_matte(Color(0.78, 0.82, 0.88), 0.2)
	# Ceiling rose + brass drop rod
	_add_mesh_cyl(root, Vector3(0, hang + 0.3, 0), 0.14, 0.035, mat_rose, false)
	_add_mesh_cyl(root, Vector3(0, hang + 0.26, 0), 0.07, 0.03, mat_br_d, false)
	_add_mesh_cyl(root, Vector3(0, hang + 0.08, 0), 0.02, 0.3, mat_br, false)
	# Central brass body (hero mass — not glass stack)
	_add_mesh_cyl(root, Vector3(0, hang - 0.02, 0), 0.09, 0.04, mat_br_d, false)
	_add_mesh_cyl(root, Vector3(0, hang - 0.1, 0), 0.14, 0.1, mat_br, false)
	_add_mesh_cyl(root, Vector3(0, hang - 0.18, 0), 0.09, 0.04, mat_br_l, false)
	# Double brass arm rings (gasolier identity)
	_add_mesh_cyl(root, Vector3(0, hang - 0.06, 0), 0.3, 0.018, mat_br_d, false)
	_add_mesh_cyl(root, Vector3(0, hang - 0.1, 0), 0.34, 0.014, mat_br, false)
	# Six arms + SHORT open-mouth bells (brass gallery dominates silhouette)
	for i in 6:
		var a := float(i) * 60.0
		var rad := deg_to_rad(a)
		var ax: float = cos(rad) * 0.4
		var az: float = sin(rad) * 0.4
		# Arm: brass tube segments
		_add_mesh_box(root, Vector3(ax * 0.32, hang - 0.05, az * 0.32), Vector3(0.15, 0.03, 0.03), mat_br)
		_add_mesh_box(root, Vector3(ax * 0.65, hang - 0.1, az * 0.65), Vector3(0.13, 0.026, 0.026), mat_br_d)
		# Cup + nozzle (brass mass at shade top)
		_add_mesh_cyl(root, Vector3(ax, hang - 0.14, az), 0.04, 0.04, mat_br, false)
		_add_mesh_cyl(root, Vector3(ax, hang - 0.17, az), 0.055, 0.022, mat_br_d, false)
		_add_mesh_cyl(root, Vector3(ax, hang - 0.19, az), 0.03, 0.02, mat_br_l, false)
		# SHORT wide-open bell (h=0.08 — not tall cone mass)
		var shade := MeshInstance3D.new()
		var sm := CylinderMesh.new()
		sm.top_radius = 0.035
		sm.bottom_radius = 0.095
		sm.height = 0.08
		sm.radial_segments = 12
		shade.mesh = sm
		shade.material_override = mat_shade
		shade.position = Vector3(ax, hang - 0.24, az)
		root.add_child(shade)
		...
		inner.position = Vector3(ax, hang - 0.23, az)
		root.add_child(inner)
		# Double brass gallery at mouth (breaks cream-cone read)
		_add_mesh_cyl(root, Vector3(ax, hang - 0.28, az), 0.09, 0.012, mat_br_d, false)
		_add_mesh_cyl(root, Vector3(ax, hang - 0.295, az), 0.08, 0.01, mat_br, false)
		# Flame
		...
		_fl7.emission_energy_multiplier = 1.8
		_fl7.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
		_add_mesh_cyl(root, Vector3(ax, hang - 0.18, az), 0.02, 0.025, _fl7, false)
	# Crystal drops under central bowl
	for j in 8:
		var ja := float(j) * 45.0 + 15.0
		var jr := deg_to_rad(ja)
		_add_mesh_box(
			root,
			Vector3(cos(jr) * 0.09, hang - 0.28, sin(jr) * 0.09),
			Vector3(0.012, 0.08, 0.012),
			mat_crystal
		)
		_add_mesh_cyl(root, Vector3(cos(jr) * 0.09, hang - 0.33, sin(jr) * 0.09), 0.012, 0.02, mat_crystal, false)
	var light := OmniLight3D.new()
	light.light_color = Color(1.0, 0.86, 0.58)
	light.light_energy = 1.3
	light.omni_range = 8.0
	light.position = Vector3(0, hang - 0.18, 0)
	root.add_child(light)
	return root
