# CURRENT — Aetheric Engine (agent one-screen)

**Updated:** 2026-07-29 ~overnight loop 145  
**HEAD:** (pending commit) visual loop 145  
**10m density scheduler:** OFF  
**Do not mark goal complete after one pass** — living priority queue.

## Ordered work list

| # | Item | Status |
|---|------|--------|
| 1–3 | Spacing / collision / walk | **DONE 67–68** |
| 4 | Unique Victorian objects | **ADVANCED 71–128** plants |
| 5 | Geometric / surface detail | **ADVANCED 72–116** paths |
| 6 | Victorian wall art | **ADVANCED 69+96+97+133** kitchen |
| 7 | Plant diversity / side volume | **ADVANCED 74–141** palm fronds |
| 8 | Side-view mesh furniture | **ADVANCED 70–144** sofa bulk |
| 9 | Door transition safety | **DONE tests** 76+98+125+137 |
| 10 | Wallpaper contrast | **ADVANCED 72–142** morning stripes |
| 11 | Hall / prep still-life | **ADVANCED 73–145** prep kits |
| 12–23 | Conservatory / gallery / workshop densify | **ADVANCED 116–143** rug clean |
| 24–52 | Materials…stools | **DONE 89–127+138** |
| 53–62 | Copper…stools/sconces | **DONE 118–127+140** |
| 63–72 | Plants…tight bulk | **DONE 128–137** |
| 73–78 | Sofa / chalk / plants / mirror / wing / rug | **DONE 138–143** |
| 79 | Fireplace + hall polish + sofa bulk | **DONE 144** |
| 80 | Kitchen prep still-life (boards/food/tools) | **DONE 145** |

## Loops

| Loop | Work |
|------|------|
| 143 | Cross-plane bulk legs off + short table feet (gallery L-junk) |
| 144 | Fireplace overmantel/fender/tools + sofa bulk + hall table legs |
| 145 | **Kitchen prep:** rewrite still-life kits — dough board, loaf, apples, knife, shallow basin, cloth sack; baking/veg/herb kits; lower shelf boxes not copper towers |

## Still high-value if continuing

- Human E-door playtest confirmation (automated suite PASS)  
- Optional: hall chair plant-card bleed residual (cross-plane tradeoff)  
- Optional: paper-thin chairs edge-on without bulk (tradeoff accepted)  
- Optional: workshop bench-top micro (files/clamps/scrap timber) if playtest flags  
- Optional: floor basket weave read (still slightly cylindrical at distance)  

## Anchors

- Door test: `scripts/test_door_transitions.gd`  
- Shots: `auto_loop_67/` … `auto_loop_145/`  
