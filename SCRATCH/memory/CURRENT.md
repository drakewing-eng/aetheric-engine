# CURRENT — Aetheric Engine (agent one-screen)

**Updated:** 2026-07-29 ~overnight loop 167  
**HEAD:** `85f56de` visual loop 167  
**10m density scheduler:** OFF  
**Do not mark goal complete after one pass** — living priority queue.

## Ordered work list

| # | Item | Status |
|---|------|--------|
| 1–3 | Spacing / collision / walk | **DONE 67–68** |
| 4 | Unique Victorian objects | **ADVANCED 71–166** books |
| 5 | Geometric / surface detail | **ADVANCED 72–165** wing |
| 6 | Victorian wall art | **ADVANCED 69+96+97+133** kitchen |
| 7 | Plant diversity / side volume | **ADVANCED 74–146** hall clear |
| 8 | Side-view mesh furniture | **DONE 70–165** wing envelope |
| 9 | Door transition safety | **DONE tests** 76–167 portal |
| 10 | Wallpaper contrast | **ADVANCED 72–142** morning stripes |
| 11 | Hall / prep still-life | **ADVANCED 73–163** cloth sack |
| 12–23 | Conservatory / gallery / workshop densify | **ADVANCED 116–166** table |
| 24–52 | Materials…stools | **DONE 89–127+138** |
| 53–62 | Copper…stools/sconces | **DONE 118–127+140** |
| 63–72 | Plants…tight bulk | **DONE 128–137** |
| 73–101 | Sofa…conservatory folios | **DONE 138–166** |
| 102 | Door portal identity (not closet) | **DONE 167** |

## Loops

| Loop | Work |
|------|------|
| 165 | Wing single side envelope; tea cylinders only |
| 166 | Conservatory botanical table + thin leather folios |
| 167 | **Doors:** double leaves (wide), destination Label3D plaque, warm under-leaf glow + omni, overdoor cornice; prompt `[E] Enter · Room`. Door tests PASS. |

## Still high-value if continuing

- Human E-door playtest confirmation (suite + visual identity improved 167)  
- Optional: user-driven residual notes after playtest  
- Optional: kitchen flour sack still slightly cup-like mid-FOV  

## Anchors

- Door test: `scripts/test_door_transitions.gd`  
- Shots: `auto_loop_67/` … `auto_loop_167/`  
