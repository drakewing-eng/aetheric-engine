# CURRENT — Aetheric Engine (agent one-screen)

**Updated:** 2026-07-29 ~overnight loop 176  
**HEAD:** `14deb67` visual loop 176  
**10m density scheduler:** OFF  
**Do not mark goal complete after one pass** — living priority queue.

## Ordered work list

| # | Item | Status |
|---|------|--------|
| 1–3 | Spacing / collision / walk | **DONE 67–68** |
| 4 | Unique Victorian objects | **ADVANCED 71–176** range |
| 5 | Geometric / surface detail | **ADVANCED 72–176** flue |
| 6 | Victorian wall art | **ADVANCED 69+96+97+133** kitchen |
| 7 | Plant diversity / side volume | **ADVANCED 74–169** min size |
| 8 | Side-view mesh furniture | **DONE 70–165** wing envelope |
| 9 | Door transition safety | **ADVANCED 76–167** UX |
| 10 | Wallpaper contrast | **ADVANCED 72–142** morning stripes |
| 11 | Hall / prep still-life | **ADVANCED 73–174** linen sack |
| 12–23 | Conservatory densify | **ADVANCED 116–175** urn |
| 24–110 | Materials…urn | **DONE 89–175** |
| 111 | Kitchen range continuous flue | **DONE 176** |

## Loops

| Loop | Work |
|------|------|
| 174 | Solid copper can + matte pot + box flour sack |
| 175 | Classical amphora continuous body |
| 176 | **Range flue:** continuous iron shaft + bands + damper + cornice + pot (not grey Minecraft ziggurat) |

## Still high-value if continuing

- Human E-door playtest confirmation (suite PASS; visual cues 167)  
- Optional: freestanding oil lamp still thin stick mid-FOV  
- Optional: user-driven residual notes after playtest  

## Anchors

- Door test: `scripts/test_door_transitions.gd`  
- Shots: `auto_loop_67/` … `auto_loop_176/`  
